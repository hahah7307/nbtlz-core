...
class EchartsController extends BaseController
{
    ...
    {
        $sale_start = input('sale_start', date('Y-m') . '-01');
        $sale_end = input('sale_end', date('Y-m-d'));
        ...
        if (empty($get['sku'])) {
            ...
            $this->assign('sku', $get['sku']);
            $this->assign('sum', empty($sum) ? 100 : $sum);
            $this->assign('list', json_encode([]));
            $this->assign('quantity', 0);
        } else {
            $productModel = new ProductModel();
            $userModel = new UserModel();
            $user = AccountModel::where(['id'=>Session::get(Config::get('USER_LOGIN_FLAG')), 'status' => AccountModel::STATUS_ACTIVE])->find();
            $ecangUser = $userModel->where(['user_name' => $user['nickname']])->find();
            if ($user['super'] != 1) {
                if (empty($ecangUser)) {
                    $this->error('您还未开通有效的易仓账号!');
                }
            }

            // 判断sku是否自己在售
            foreach ($get['sku'] as $item) {
                $product = $productModel->where(['productSku' => $item])->find();
                $idList = explode(',', $product['sellerId']);
                $idList[] = $product['personSellerId'];

                if ($user['super'] != 1) {
                    if (!in_array($ecangUser['user_id'], $idList)) {
                        $this->error($item . '您没有查看权限!');
                    }
                }
            }

            $this->assign('sku', $get['sku']);
            $skuStr = '"' . implode('","', array_filter($get['sku'])) . '"';
            $postParam = ['sale_start' => $sale_start, 'sale_end' => $sale_end, 'skuStr' => $skuStr];
            $list = ApiClient::httpCurl("http://139.224.106.228/Home/Api/getSkuSaleState", "POST", $postParam);
            $listRes = json_decode($list, true);
            if ($listRes['code'] == 200) {
                $sumData = $listRes['data']['sumData'];
    ...
    public function export()
    {
        ...
        $this->assign('sku', $skuList);
        $skuStr = '"' . implode('","', array_filter($skuList)) . '"';
        $postParam = ['sale_start' => $sale_start, 'sale_end' => $sale_end, 'skuStr' => $skuStr];
        $list = ApiClient::httpCurl("http://139.224.106.228/Home/Api/getSkuSaleStateExport", "POST", $postParam);
        $listRes = json_decode($list, true);
        if ($listRes['code'] == 200) {
            $list = $listRes['data']['list'];
        } else {
