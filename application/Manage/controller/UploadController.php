...
use app\Manage\model\PlatformModel;
...
class UploadController extends Controller
{
    ...
    public function order_save_upload()
    {
        header("content-type:text/html;charset=utf-8");

        $user_account = $_POST['user_account'];
        if (!in_array($user_account, [1,3,4,5,6,7,8,9,10,12,13,14,28])) {
            echo json_encode(['code' => 0, 'msg' => '请先选择店铺']);
            exit();
        }
        ...
        //将文件从临时路径移动到磁盘
        if (move_uploaded_file($temp_name, 'upload/excel/' . $new_filename)){
            echo json_encode(['code' => 1, 'msg' => '文件上传成功', 'data' => $new_filename, 'origin' => $arr['filename'], 'user_account' => $user_account]);
            exit;
        }else{
            echo json_encode(['code' => 0, 'msg' => '文件上传失败']);
            exit;
...
//            echo json_encode(['code' => 0, 'msg' => $filename . '文件大小超过40M']);
//            exit();
//        }
        //phpinfo函数会以数组的形式返回关于文件路径的信息
        //[dirname]:目录路径[basename]:文件名[extension]:文件后缀名[filename]:不包含后缀的文件名
...
//        $new_filename = $default_title.'.'.$ext_suffix;
        ...
        if (move_uploaded_file($temp_name, 'upload/product/' . $new_filename)){
            $aliyunUpload = AlibabaCloudCredentialsWrapper::uploadFile('upload/product/' . $new_filename, 'install_video/' . $user['username'] . '/' . $new_filename);
            if ($aliyunUpload['code'] == 1) {
                $filesData = [
                    'file_url'      =>  $aliyunUpload['url'],
                    'file_path'     =>  'install_video/' . $user['username'] . '/' . $new_filename,
                    'file_name'     =>  $filename,
                    'created_id'    =>  $user['id'],
                    'created_date'  =>  date('Y-m-d H:i:s')
                ];
                $model = new ProductInstallVideoModel();
                if ($model->save($filesData)) {
